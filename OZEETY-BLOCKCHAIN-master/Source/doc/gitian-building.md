...
*Setup instructions for a gitian build of ozeety using a Debian VM or physical system.*
...
- [Building ozeety](#building-ozeety)
...
Clone the git repositories for ozeety and gitian and then checkout the ozeety version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/ozeety-crypto/ozeety.git
cd ozeety
...
Follow the instructions in [doc/release-process.md](release-process.md) in the ozeety repository
...
To build ozeety (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the ozeety repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/ozeety/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/ozeety-crypto/ozeety
...
For example:
```bash
URL=https://github.com/crowning-/ozeety.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit ozeety=${COMMIT} --url ozeety=${URL} ../ozeety/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit ozeety=${COMMIT} --url ozeety=${URL} ../ozeety/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit ozeety=${COMMIT} --url ozeety=${URL} ../ozeety/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/ozeety-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/ozeety-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/ozeety-build.assert
```
...
[ozeety/gitian.sigs](https://github.com/ozeety-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.

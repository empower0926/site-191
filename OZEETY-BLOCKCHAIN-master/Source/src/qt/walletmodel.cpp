...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 OZTG.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
static void NotifyzOZTGReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzOZTGReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzOZTGReset.connect(boost::bind(NotifyzOZTGReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzOZTGReset.disconnect(boost::bind(NotifyzOZTGReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zoztgTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}

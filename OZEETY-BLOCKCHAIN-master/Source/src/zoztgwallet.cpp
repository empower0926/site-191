...
CzOZTGWallet::CzOZTGWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zoztg seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZOZTGSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZOZTGSeed_deprecated()) {
                    LogPrintf("%s: Updated zOZTG seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zoztg seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zoztg wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzOZTGWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZOZTGCount(nCountLastUsed);
    else if (!walletdb.ReadZOZTGCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzOZTGWallet::Lock()
{
    seedMaster = 0;
}

void CzOZTGWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzOZTGWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZOZTG(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzOZTGWallet::LoadMintPoolFromDB()
{
...
void CzOZTGWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzOZTGWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzOZTGWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zoztgTracker->HasPubcoinHash(pMint.first)) {
...
bool CzOZTGWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZOZTG(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zoztgTracker which also adds to database
    pwalletMain->zoztgTracker->Add(dMint, true);
...
void CzOZTGWallet::SeedToZOZTG(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzOZTGWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzOZTGWallet::GenerateDeterministicZOZTG(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzOZTGWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzOZTGWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{

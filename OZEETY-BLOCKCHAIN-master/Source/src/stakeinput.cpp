...
// The zOZTG block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zOZTG is a hash of the serial
...
{
    //Create an output returning the zOZTG that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZOZTGOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zOZTG output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zOZTG", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZOZTGOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zOZTG output", __func__);
...
{
    CzOZTGTracker* zoztgTracker = pwallet->zoztgTracker.get();
    CMintMeta meta;
    if (!zoztgTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zoztgTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}

...
/** Fees smaller than this (in uoztg) are considered zero fee (for relaying and mining)
...
{
    //Check to see if the zOZTG is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zOZTG spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zOZTG without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zOZTG spend with serial %s is already in block %d\n", __func__,
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zOZTG spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zOZTG spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zoztg");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zOZTG mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
     nSubsidy = 730000000 * COIN;
    }
    if (nHeight <= Params().LAST_POW_BLOCK() && nHeight > 0) {
     nSubsidy = 25 * COIN;
    }
    else if (nHeight > Params().LAST_POW_BLOCK() && nHeight <= 265000) {
     nSubsidy = 25 * COIN;
    }else if (nHeight > 265000 && nHeight <= 1000000) {
        nSubsidy = 20 * COIN;
	} else if (nHeight > 1000000 && nHeight <= 1800000) {
        nSubsidy = 15 * COIN;
	} else if (nHeight > 1800000 && nHeight <= 2500000) {
        nSubsidy = 10 * COIN;
	} else if (nHeight > 2500000 && nHeight <= 6000000) {
        nSubsidy = 5 * COIN;
	} else {
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZOZTGStake)
{
    int64_t ret = 0;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from ozeety
...
{
    RenameThread("ozeety-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZOZTGMinted()
{
...
void RecalculateZOZTGSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zOZTG supply
...
bool RecalculateOZTGSupply(int nHeightStart)
{
...
bool UpdateZOZTGSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateOZTGSupply(Params().Zerocoin_StartHeight());
    }

    //Track zOZTG money supply in the block index
    if (!UpdateZOZTGSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zOZTG supply for block=%s height=%d", __func__,
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert OZTG to zOZTG
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zOZTG spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zOZTG serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zOZTG stake block is less than allowed start height", __func__);

    if (CZPivStake* zOZTG = dynamic_cast<CZPivStake*>(stake)) {
        CBlockIndex* pindexFrom = zOZTG->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zOZTG stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zOZTG stake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zOZTG->GetValue()));
        if (nChecksum200 != zOZTG->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zOZTG->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZOZTG() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zOZTG fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zOZTG mints and %d zOZTG spends\n", __func__, nMints, nSpends);

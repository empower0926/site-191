...
#ifndef ozeety_ZOZTGWALLET_H
#define ozeety_ZOZTGWALLET_H
...
public:
    CzOZTGWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZOZTG(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZOZTG(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //ozeety_ZOZTGWALLET_H

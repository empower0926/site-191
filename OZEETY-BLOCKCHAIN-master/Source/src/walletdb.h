...
public:
    ...
    bool WriteZOZTGSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZOZTGSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZOZTGSeed_deprecated(uint256& seed);
    bool EraseZOZTGSeed();
    bool EraseZOZTGSeed_deprecated();

    bool WriteZOZTGCount(const uint32_t& nCount);
    bool ReadZOZTGCount(uint32_t& nCount);

...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zoztgTracker = unique_ptr<CzOZTGTracker>(new CzOZTGTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZOZTG_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZOZTG_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
